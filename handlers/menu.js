class Menu {
  ...
  getStatistics(allCars, topBrands, topModels, avgMileage, avgPrice) {
    return `Статистика по купленным авто📊\n\nВсего автомобилей куплено: ${allCars}\n\nСамые популярные марки авто:\n🥇 ${topBrands[0]._id} - ${topBrands[0].count}\n🥈 ${topBrands[1]._id} - ${topBrands[1].count}\n🥉 ${topBrands[2]._id} - ${topBrands[2].count}\n\nСамые популярные модели авто:\n🥇 ${topModels[0]._id.brand} ${topModels[0]._id.model} - ${topModels[0].count}\n🥈 ${topModels[1]._id.brand} ${topModels[1]._id.model} - ${topModels[1].count}\n🥉 ${topModels[2]._id.brand} ${topModels[2]._id.model} - ${topModels[2].count}\n\nСредние показатели:\nСредний пробег авто - ${Math.round(avgMileage[0].average)} тыс. км\nСредняя цена авто - ${Math.round(avgPrice[0].average)} тыс. руб.`
  }
